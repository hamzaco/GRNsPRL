...
"CC_IGVH_mutations",
"CC_Lymphopoiesis_Regulatory_Network","CC_Signaling_in_Macrophage_Activation",
...
for i in range(0,len(entries)):
    ...
    for i in range(0,random_size):
        gn=shuffle_network(original,'swapping')
...
with open('p_values_swapping_coherences.json', 'w') as fp:
    json.dump(p_values, fp)
with open('coherences_swapping.json', 'w') as fp:
    json.dump(coherences_random, fp)
with open('error_bars_swapping_coherences.json', 'w') as fp:
